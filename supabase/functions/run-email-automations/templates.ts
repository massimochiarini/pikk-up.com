const SITE_URL = 'https://pikk-up.com' // Should ideally come from env, but hardcoding for v1 templates in Deno

function buildClickUrl(email: string, type: string, url: string) {
  return `${SITE_URL}/api/email/click?email=${encodeURIComponent(email)}&type=${encodeURIComponent(type)}&url=${encodeURIComponent(url)}`
}
  ...
  switch (type) {
    ...
    case 'pre_class_reminder':
      ...
      content = `
        ...
        <a href="${buildClickUrl(email, 'pre_class_reminder', `${SITE_URL}/my-classes`)}" style="display: inline-block; padding: 12px 24px; border: 1px solid #1a1a1a; color: #1a1a1a; text-decoration: none; border-radius: 6px; font-size: 14px;">View my classes</a>
      `
      break

    case 'post_class_followup':
      ...
      content = `
        ...
        <a href="${buildClickUrl(email, 'post_class_followup', `${SITE_URL}/classes`)}" style="display: inline-block; padding: 12px 24px; background: #1a1a1a; color: #fff; text-decoration: none; border-radius: 6px; font-size: 14px;">Rebook now</a>
      `
      break
    ...
    case 'rebook_nudge':
      ...
      content = `
        ...
        <a href="${buildClickUrl(email, 'rebook_nudge', `${SITE_URL}/book/${payload.classId}`)}" style="display: inline-block; padding: 12px 24px; background: #1a1a1a; color: #fff; text-decoration: none; border-radius: 6px; font-size: 14px;">Book my spot</a>
      `
      break

    default:
      subject = "Update from PickUp Yoga"
      content = `<p style="margin: 0 0 16px; font-size: 15px; color: #555; line-height: 1.6;">Hello! You have a new message from PickUp Yoga.</p>`
  }
